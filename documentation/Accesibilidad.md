...
- **`SelectPremium` con `aria-label` = propósito + valor (WCAG 4.1.2)**: cuando un `SelectPremium` recibe `aria-label` sin label visible (filtros del dashboard), el combobox expone `aria-label = "{propósito}: {valor seleccionado}"` (p. ej. «Filtrar por rango de tiempo: Últimos 7 días»). Antes el atributo se perdía en `{...props}` sobre el contenedor y el control caía al placeholder, haciendo indistinguibles los tres filtros y ocultando su valor. Documentado en `Architecture_Decisions.md` (ADR-194).
- **Modales cierran con Escape (WCAG 2.1.2 / modal-escape)**: convención para todos los diálogos. Corregido el modal «Añadir cartas» de la edición de mazos, que cerraba con click-fuera y «X» pero no con teclado; ahora `role="dialog"` + `aria-modal` + `aria-labelledby` + Escape (ADR-194).
- **Contrato de modal accesible y hook `useModalA11y` (ADR-200)**: todo diálogo custom (no solo `ConfirmationModal`) debe declarar `role="dialog"` + `aria-modal="true"` + `aria-labelledby`, atrapar el foco con un ciclo Tab (first↔last), enfocar el primer campo al abrir, cerrar con Escape (salvo durante un envío en curso), bloquear el scroll del body y restaurar el foco al elemento previo al cerrar. El patrón está centralizado en `frontend/src/hooks/useModalA11y.js` — `useModalA11y({ isOpen, onClose, panelRef, initialFocusRef, escapeDisabled })`. Aplicado a **todos** los diálogos custom: los dos modales de `StudentManagement` (creación/edición de alumnos = datos de menores), `MfaChallengeModal` (2FA), `CreateContextModal`, `UploadAssetModal` y el selector de alumno de `SessionDetail`, que además migraron sus controles a `ButtonPremium`/`InputPremium`. Los diálogos destructivos siguen pasando por `ConfirmationModal` (que ya encapsula el mismo contrato). Nota de implementación: el hook hace `preventDefault` en Escape y restaura el foco al elemento previo; para modales que se cierran con un callback derivado (p. ej. 2FA con `close(true)`), envolver el callback en `useCallback` para no re-suscribir el efecto en cada render.
