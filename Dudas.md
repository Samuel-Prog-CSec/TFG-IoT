...
15. Actualmente, el modelo 'GameContext' referencia a un único 'GameMechanic'. ¿Deberíamos permitir que un 'GameContext' pueda estar asociado a múltiples 'GameMechanics' para mayor flexibilidad en la creación de sesiones de juego? Por ejemplo, el contexto "Geografía" podría ser utilizado tanto para una mecánica de "Asociación" como para una de "Secuencia".
  - 15.1 Si permitimos múltiples 'GameMechanic' para un 'GameContext', ¿todos los contextos deben ser compatibles con todas las mecánicas? ¿O debemos definir reglas de compatibilidad específicas? ¿Cómo gestionamos estas reglas en el backend?
    - 15.1.1 En caso de que permitamos múltiples 'GameMechanic' para un 'GameContext', ¿cómo afectaría esto a la creación y gestión de 'GameSessions'? ¿Necesitaríamos un proceso adicional para validar que la combinación de 'GameContext' y 'GameMechanic' es válida al crear una sesión de juego?
    - 15.1.2 En caso de que NO todos los contextos deban ser compatibles con todas las mecánicas, ¿cómo definimos y gestionamos estas incompatibilidades? ¿Las gestionamos nosotros o los profesores?
16. ¿Pueden haber varias partidas activas simultáneamente dentro de una misma 'GameSession' para permitir que distintos jugadores puedan juegar y no esperar turnos? Si es así, ¿cómo gestionamos la sincronización y el estado compartido de sesión entre estas partidas?
17. En el modelo 'GamePlay', ¿deberíamos incluir un campo para registrar la duración total de la partida o el tiempo empleado por cada jugador? Actualmente, sólo manejamos el tiempo máximo por ronda para manejar timeouts, pero no tenemos un seguimiento del tiempo total de juego.
18. Cuando se crea una 'GameSession', ¿cómo manejamos la creación de partidas? ¿Se crean todas las partidas de una vez o se generan dinámicamente a medida que los jugadores se unen o avanzan en el juego?
